...
class RolloutBaseline:
	def __init__(self, model, task, weight_dir, dump_date, n_rollout_samples = 10000, 
	...
	def _update_baseline(self, model, epoch):
		...
		if self.from_checkpoint and self.alpha == 0:
			...
			# For checkpoint
			if not os.path.exists('%s%s/%s/baseline/' % (self.weight_dir, self.task, self.dump_date)):
				os.makedirs('%s%s/%s/baseline/' % (self.weight_dir, self.task, self.dump_date))
			torch.save(self.model.state_dict(), '%s%s/%s/baseline/train_epoch%s.pt' % (self.weight_dir, self.task, self.dump_date, epoch))
	...
	def eval_all(self, dataset):
		...
		val_costs = self.rollout(self.model, dataset, batch = 256)
