...
if __name__ == "__main__":
    ...
    step = 5
    datasets = ["tsp{}_uniform_test_seed1234.pkl".format(i) for i in range(start, end+step, step)]

    results_list = []
    for width in widths:
        for dataset_path in datasets:
            costs, _, _ = eval_dataset("../data/tsp/" + dataset_path, width, opts.softmax_temperature, opts)
            results_list.append(np.mean(costs))
            print(results_list)
    print(results_list)
